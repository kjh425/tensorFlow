...
def train_step(model, x, y):
    with tf.GradientTape() as tape:
        y_pred, logits = model(x, training=True)
...
for i in range(10000):
  ...
  if i % 100 == 0:
    train_accuracy = compute_accuracy(CNN_model(batch_x, training=False)[0], batch_y)
    loss_print = cross_entropy_loss(CNN_model(batch_x, training=False)[1], batch_y)
...
for i in range(10):
  ...
  test_accuracy = test_accuracy + compute_accuracy(CNN_model(batch_x, training=False)[0], test_batch_y).numpy()
